...
# 		sys.exit(1)
	...
	for idx, val in enumerate(listoffiles):
		for x in listoffiles[idx+1:]:
			proc = mp.Process(target=Calculate, args=(val,x,listofresults))
			pool.apply(proc)



	listofresults.sort(key=operator.attrgetter('normal'))
	print "Normal" + "\t:\t" + "Kabsch" + "\t:\t" + "Quater" + "\t:\t" + "File 1" + "\t:\t" + "File 2"
	for res in listofresults:		
		if res.quater < 2: print str(res.normal) + "\t:\t" + str(res.kabsch) + "\t:\t" + str(res.quater) + "\t:\t" + str(res.filename1) + "\t:\t" + str(res.filename2)
