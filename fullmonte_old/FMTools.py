...
# EXECECTUBALE ################################################
G09_EXEC = '/opt/gaussian09d/g09'
MOPAC_EXEC = '/opt/mopac/MOPAC2016.exe'
###############################################################
	...
	def __init__(self, MolSpec, instruct, log):
		...
		if instruct != "default":
			...
			for line in instructlines:
				...
				if 	line.find("THRD") > -1: self.THRD = int((line.split("=")[1]).rstrip('\n').lstrip())
...
class writeInput:
	...
	def __init__(self, JobSpec, MolSpec):
		...
		def writeMopac(self, JobSpec, MolSpec):
			...
			fileout.write(JobSpec.JOBTYPE+" charge="+str(MolSpec.CHARGE)+" threads="+str(JobSpec.THRD))
