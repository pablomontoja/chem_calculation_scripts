...
c
     ...
     ?ro6lj(len),ro12lj(len),dro6(len),dro12(len)
      ...
      character(50) runfilename
      call getarg(1, runfilename)

      if ( runfilename == '' ) then
        runfilename='mobcal.run'
      endif
c
c     ***************************************************************
c
...
c      dipol=1.265d-30/(2.d0*4.d0*pi*xeo)
      dipol=dipol*xe*xe 
      write(8,601) dipol
...
c 2011 pcharge(iatom)=1.d0/dfloat(inatom)
 2011 pcharge(iatom)=tcharge/dfloat(inatom)
      endif
c
...
c
c      if(dchar.eq.'calc') write(8,615) tcharge,acharge
...
c
      ...
      eogas=0.06900d0
      rogas=3.6600d0
c     convert epsilon from kcal/mol to meV
...
c     and ro from fitting mobilities of C6H6 and others) 
      ...
      itest=1
      xmass(iatom)=1.008d0
      eolj(iatom)=dsqrt(eogas*0.0189)*conve*xe
      rolj(iatom)=dsqrt(rogas*1.2409)*convr*1.0d-10
...
c
      ...
      itest=1
      xmass(iatom)=12.01d0
      eolj(iatom)=dsqrt(eogas*0.0977)*conve*xe
      rolj(iatom)=dsqrt(rogas*3.5814)*convr*1.0d-10
...
c
      ...
      itest=1
      xmass(iatom)=14.01d0
      eolj(iatom)=dsqrt(eogas*0.0828)*conve*xe
      rolj(iatom)=dsqrt(rogas*4.3920)*convr*1.0d-10
c      eolj(iatom)=0.112982299*xe
c      rolj(iatom)=3.762562956*0.890898718*1.0d-10
      rhs(iatom)=2.7d0*1.0d-10
      endif
c     
...
c
      ...
      itest=1
      xmass(iatom)=16.00d0
      eolj(iatom)=dsqrt(eogas*0.0558)*conve*xe
      rolj(iatom)=dsqrt(rogas*3.2550)*convr*1.0d-10
c      eolj(iatom)=0.105356537*xe
c      rolj(iatom)=3.679402125*0.890898718*1.0d-10
      rhs(iatom)=2.7d0*1.0d-10
      endif
c
...
c
      ...
      itest=1
      xmass(iatom)=22.99d0
      eolj(iatom)=dsqrt(eogas*0.03000)*conve*xe
      rolj(iatom)=dsqrt(rogas*2.9830)*convr*1.0d-10
      rhs(iatom)=2.853d0*1.0d-10
      endif
c
c     lithium (same as silicon)
c
...
c
      ...
      itest=1
      xmass(iatom)=28.09d0
      eolj(iatom)=dsqrt(eogas*0.4020)*conve*xe
      rolj(iatom)=dsqrt(rogas*4.2950)*convr*1.0d-10
...
c
      ...
      itest=1
      xmass(iatom)=32.06d0
      eolj(iatom)=dsqrt(eogas*0.2740)*conve*xe
      rolj(iatom)=dsqrt(rogas*4.0350)*convr*1.0d-10
c      eolj(iatom)=0.225144398*xe
c      rolj(iatom)=3.950617673*0.890898718*1.0d-10
      rhs(iatom)=3.5d0*1.0d-10
      endif
c
...
c
      ...
      itest=1
      xmass(iatom)=55.85d0
      eolj(iatom)=dsqrt(eogas*0.0130)*conve*xe
      rolj(iatom)=dsqrt(rogas*2.9120)*convr*1.0d-10
      rhs(iatom)=3.5d0*1.0d-10
      endif
c
c     Phosphorous
c
      ...
      itest=1
      xmass(iatom)=30.97d0
      eolj(iatom)=dsqrt(eogas*0.305)*conve*xe
      rolj(iatom)=dsqrt(rogas*4.1470)*convr*1.0d-10
...
c
      ...
      itest=1
      xmass(iatom)=7.00d0
      eolj(iatom)=dsqrt(eogas*0.0233)*conve*xe
      rolj(iatom)=dsqrt(rogas*2.2794)*convr*1.0d-10
      rhs(iatom)=2.7d0*1.0d-10
      endif
c
c     potassium
...
c
      e=0.5d0*mu*(dw(1)**2+dw(3)**2+dw(5)**2)
      erat=(e+pot)/etot
c      if(erat.lt.1.01d0.and.erat.gt.0.99d0) return
c      write(8,104) erat,v,b
c  104 format(/1x,'energy not conserved: e ratio =',1pe11.4,
c     ?' v =',e11.4,' b =',e11.4)
c      write(8,109) 0.5d0*mu*v*v/eo,theta*cang,phi*cang,
c     ?gamma*cang
c  109 format(1x,'gst2 =',1pe11.4,' theta =',e11.4,' phi =',
c     ?e11.4,' gamma =',e11.4)
c      if(ip.eq.1) write(8,102)
c  102 format()
c      ifailc=ifailc+1
c      if(ifailc.eq.ifail) stop
      return
c       
c
      end
